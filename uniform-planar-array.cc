...
#include <sstream> // Added for string parsing/serialization
#include <numeric> // Added for std::accumulate
#include <ns3/string.h> // Added for MakeStringAccessor
...
UniformPlanarArray::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::UniformPlanarArray")
            ...
            .AddAttribute("PortPowerAllocation",
                            "Semicolon-separated list of power scaling factors for each antenna port (linear scale)",
                            StringValue(""),
                            MakeStringAccessor(&UniformPlanarArray::SetPortPowerAllocationString,
                                               &UniformPlanarArray::GetPortPowerAllocationString),
                            MakeStringChecker());
...
void
UniformPlanarArray::SetPortPowerAllocationString(const std::string& value){

    NS_LOG_FUNCTION(this << value);
    if (value.empty())
    {
        m_portPowerAllocation.clear();
        return;
    }
    std::vector<double> powerAllocation;
    ...
    while (std::getline(iss, item, ';'))
    {
        try
        {
            ...
            powerAllocation.push_back(val);
        }
        catch (...)
        {
            NS_FATAL_ERROR("Invalid power allocation value: " << item);
        }
    }
    SetPortPowerAllocation(powerAllocation);


}

std::string
UniformPlanarArray::GetPortPowerAllocationString() const
{
    std::ostringstream oss;
    for (size_t i = 0; i < m_portPowerAllocation.size(); ++i)
    {
        if (i > 0)
        {
            oss << ";";
        }
        oss << m_portPowerAllocation[i];
    }
    return oss.str();
}

void
UniformPlanarArray::SetPortPowerAllocation(const std::vector<double>& powerAllocation)
{
    NS_LOG_FUNCTION(this << powerAllocation.size());
    uint16_t expectedPorts = GetNumPorts();
    NS_ASSERT_MSG(powerAllocation.size() == expectedPorts,
                  "Power allocation vector size (" << powerAllocation.size()
                  << ") does not match number of ports (" << expectedPorts << ")");
    double sum = std::accumulate(powerAllocation.begin(), powerAllocation.end(), 0.0);
    NS_ASSERT_MSG(std::abs(sum - 1.0) < 1e-6, "Power allocation factors must sum to 1.0");
    m_portPowerAllocation = powerAllocation;
}

const std::vector<double>&
UniformPlanarArray::GetPortPowerAllocation() const
{
    return m_portPowerAllocation;
}
