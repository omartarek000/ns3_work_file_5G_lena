...
class UniformPlanarArray : public PhasedArrayModel
{
  public:
    ...
    /** 
     * declare the function that returns the port power allocation
     * and sets the power allocation for each port
    */
   const std::vector<double>& GetPortPowerAllocation() const ;
   void SetPortPowerAllocation(const std::vector<double>& portPowerAllocation);
  ...
  private:
    ...
    std::vector<double> m_portPowerAllocation;     //!< the power allocation for each port

    // attribute accessors for port power allocation
    void SetPortPowerAllocationString(const std::string& value);
    std::string GetPortPowerAllocationString() const;
};
